...
public static class TorProxyExtensions
{
    public static async Task ConfigureAndStartAsync(this ITorProxy proxy)
    {
...
public interface ITorProxy : IDisposable
{
...
public class TorProxy : ITorProxy
{
    ...
    public async Task ConfigureAsync()
    {
        if (!_configured)
        {
            ...
            await ConfigureToolAsync(_tor!, new TorConfigurationDictionary()).ConfigureAwait(false);
            if (!_settings.PrivoxySettings.Disable)
            {
                await ConfigureToolAsync(_privoxy!, new PrivoxyConfigurationDictionary()).ConfigureAwait(false);
            }
    ...
    public async Task StartAsync()
    {
        ...
        if (!_started)
        {
            ...
            if (!_settings.PrivoxySettings.Disable)
            {
                await _toolRunner.StartAsync(_privoxy!).ConfigureAwait(false);
            }
    ...
    /// <summary>
    /// Returns a connected and authenticated Tor control client for the running Tor process. The caller is
    ...
    private async Task<Tool> ConfigureToolAsync(Tool tool, IConfigurationDictionary configurationDictionary)
    {
    ...
    private bool WaitForConnect(string host, int port) =>
        WaitForConnect(host, port, _settings.WaitForConnect);
