...
internal static class ToolUtility
{
    ...
    public static ToolSettings GetPrivoxyToolSettings(TorProxySettings settings)
    {
        ...
        {
            string? prefix = null;
            if (settings.Architecture == TorProxyArchitecture.X86)
            {
                prefix = "privoxy-linux32-";
            }
            else if (settings.Architecture == TorProxyArchitecture.X64)
            {
    ...
    public static ToolSettings GetTorToolSettings(TorProxySettings settings)
    {
        if (settings.OSPlatform == TorProxyOSPlatform.Windows)
        {
            return new ToolSettings
            {
                Name = TorName,
                Prefix = settings.Architecture == TorProxyArchitecture.X64 ? "tor-win64-" : "tor-win32-",
    ...
    /// <summary>
    /// Read the <see cref="TorProxySettings.ZippedToolsDirectory"/> and find the latest tool matching the criteria
    /// in the provided <paramref name="toolSettings"/>. If none is found return null.
    /// </summary>
    /// <param name="settings">The settings for TorProxy.</param>
    /// <param name="toolSettings">The settings for the tool.</param>
    /// <returns>The tool, or null if none was found.</returns>
    public static Tool? GetLatestToolOrNull(TorProxySettings settings, ToolSettings toolSettings)
    {
