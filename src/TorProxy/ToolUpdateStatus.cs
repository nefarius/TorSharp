...
public enum ToolUpdateStatus
{
    ...
    /// <summary>
    /// There is no version available locally. In this case, <see cref="TorProxy"/> will fail to run if no
    /// tool is downloaded and put into the <see cref="TorProxySettings.ZippedToolsDirectory"/>.
    /// </summary>
    NoLocalVersion,

    /// <summary>
    /// A version is available locally but a newer version is available remotely. In this case,
    /// <see cref="TorProxy"/> will run using this existing local version if no newer version is downloaded.
    /// </summary>
