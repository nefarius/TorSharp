...
/// <summary>
/// Fetches and deserializes the TorProxy mirror manifest, then resolves
/// <see cref="DownloadableFile"/> entries for Tor and Privoxy based on the
/// current <see cref="TorProxySettings"/> platform / architecture.
/// </summary>
internal class MirrorManifestFetcher
{
    ...
    public MirrorManifestFetcher(HttpClient httpClient, TorProxySettings settings)
    {
    ...
    // -----------------------------------------------------------------------
    // Static helpers — map TorProxySettings platform/arch to manifest keys
    // -----------------------------------------------------------------------
    ...
    /// </summary>
    public static DownloadableFile? GetTorEntry(MirrorManifest manifest, TorProxySettings settings)
    {
    ...
    /// if the current platform/architecture is not present in the manifest.
    /// </summary>
    public static DownloadableFile? GetPrivoxyEntry(MirrorManifest manifest, TorProxySettings settings)
    {
        ...
        var format = settings.OSPlatform == TorProxyOSPlatform.Windows
    ...
    private static string? GetTorKey(TorProxySettings settings)
    {
        var os = settings.OSPlatform switch
        {
            TorProxyOSPlatform.Windows => "windows",
            TorProxyOSPlatform.Linux => "linux",
            _ => null,
        };
        ...
        {
            TorProxyArchitecture.X86 => "x86",
            TorProxyArchitecture.X64 => "x86_64",
            _ => null,
        };
    ...
    private static string? GetPrivoxyKey(TorProxySettings settings)
    {
        ...
        if (settings.OSPlatform == TorProxyOSPlatform.Linux)
        {
            return settings.Architecture switch
            {
                TorProxyArchitecture.X86 => "linux-i386",
                TorProxyArchitecture.X64 => "linux-amd64",
                _ => null,
            };
        }
