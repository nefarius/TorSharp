...
namespace Nefarius.Utilities.TorProxy.Tests
{
    public class TestAppTests : IClassFixture<TestAppFixture>
    {
        ...
        private static string GetTestAppProjectDir()
        {
            ...
            while (current != null && !Directory.EnumerateFiles(current, "TorProxy.sln").Any())
            {
            ...
            if (current == null)
            {
                throw new InvalidOperationException($"Could not find the repository root by probing upwards for TorProxy.sln. Current directory: {Directory.GetCurrentDirectory()}");
            }
