...
namespace Nefarius.Utilities.TorProxy.Tests.TestSupport
{
    public class HttpFixture
    {
        ...
        public HttpFixture()
        {
            var cacheDirectory = Environment.GetEnvironmentVariable("TORPROXY_TEST_CACHE");
            if (string.IsNullOrWhiteSpace(cacheDirectory))
            {
                cacheDirectory = Path.Combine(
                    Path.GetTempPath(),
                    "Nefarius.Utilities.TorProxy.Tests",
                    "cache");
            }
        ...
        public TorProxyToolFetcher GetTorProxyToolFetcher(ITestOutputHelper output, TorProxySettings settings, HttpClient httpClient)
        {
            return new TorProxyToolFetcher(
                settings,
                httpClient,
